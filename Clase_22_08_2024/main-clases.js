import { agregarEventoInput } from "./agregarEventoInput.js";
import { agregarInputNombre } from "./agregarInputNombre.js";
import { agregarSelectCargos } from "./agregarSelectCargos.js";
import { agregarRut } from "./agregarSelectRut.js";
import { agregarEventoChange } from "./changeRuts.js";
...
miForm.addEventListener("submit", function (event) {
  ...
  // AGREGAMOS UN NUEVO RUT A NUESTRO SELECT
  const select = document.getElementById("ruts");
  const option = document.createElement("option");
  option.value = rut;
  option.textContent = rut;
  select.appendChild(option);

  // AGREGAMOS UN NUEVO CARGO A NUESTRO SELECT
  ...
  selectCargos.appendChild(opcionPorDefecto);

  const cargos = miEmpresa.listarPersonal().map((persona) => {
    return persona.cargo;
  });

  const cargosSet = new Set(cargos);
  const cargosSinRepetidos = [...cargosSet];

  cargosSinRepetidos.forEach((cargo) => {
    const optionCargo = document.createElement("option");
    optionCargo.value = cargo;
    optionCargo.textContent = cargo;
    selectCargos.appendChild(optionCargo);
  });

  const dataList = document.getElementById("listado");
  const optionData = document.createElement("option");
  optionData.value = nombre;
  optionData.textContent = nombre;
...
agregarEventoChange(miEmpresa);

agregarSelectCargos();

agregarEventoChangeCargos(miEmpresa);

// agregarInputNombre();
agregarEventoInput(miEmpresa);
